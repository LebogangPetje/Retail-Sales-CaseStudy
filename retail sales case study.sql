...
--- age bucket
    ...
    ---bucket for the purchase amount 
   CASE WHEN purchase_amount < 30 THEN 'Low'
    ...
    ---bucket for the revuew rating (look at figures in comments after code)
   CASE WHEN review_rating < 3 THEN 'Poor'
    ...
    -- purchase price bucket 
    CASE WHEN previous_purchases BETWEEN 1 AND 10 THEN 'Low'
    ...
    --- frequency bucket 
    CASE WHEN(frequency_of_purchases) IN ('weekly','fortnightly','bi-weekly','monthly') THEN 'Frequent Buyer'
        WHEn (frequency_of_purchases) IN ('every 3 months','annual quarterly') THEN 'Occasional Buyer'
    ELSE 'Unspecified'
    END AS purchase_frequency_category,

    --shipping bucket
    CASE WHEN (shipping_type) IN ('standard','store pick up','free shipping') THEN 'Normal Shipping'
        WHEN (shipping_type) IN ('express','next day air','2 day shipping') THEN 'Fast Shipping'
    ELSE 'Unspecified'
    END AS shipping_category,

    -- nulls in dataset (strings only)(nb to remove them from select if already processed here) (double check data)
